...
export default Form.create({})({
  ...
  data() {
    return {
      modalProps: {
        width: 700,
        wrapClassName: 'bnm-modal-eidt-function-form'
      },
      tableProps: {
        columns: [
          {
  ...
  watch: {
    ...
    visible: {
      ...
      async handler(value) {
        if (value) {
          this.getDetail({ id: this.currentItem.id, moduleName: this.moduleName })
        } else {
            ...
            <Col span={24}>
              <Form.Item label="所属菜单">
                {this.form.getFieldDecorator('menuId', {
                  ...
                  rules: [{ required: true, type: 'array', message: '请输入菜单!', trigger: 'change' }]
                })(<CascaderMenu />)}
              </Form.Item>
            </Col>
            <Col span={12}>
                ...
                scopedSlots={{
                  fnUrl: (text, record, index) => (
                    <Form.Item label="" style={{ 'margin-bottom': '0' }}>
                      {this.form.getFieldDecorator(`fnUrl${index}`, {
                        initialValue: record.fnUrl
                      })(<Input placeholder="请输入" allowClear />)}
                    </Form.Item>
                  ),
                  fnInfoDescribe: (text, record, index) => (
                    <Form.Item label="" style={{ 'margin-bottom': '0' }}>
                      {this.form.getFieldDecorator(`fnInfoDescribe${index}`, {
                        initialValue: record.fnInfoDescribe
                      })(<Input placeholder="请输入" allowClear />)}
                    </Form.Item>
                  ),
